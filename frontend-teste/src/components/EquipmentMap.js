...
const EquipmentMap = forwardRef(
	(
		{ positions, equipments, states, stateHistory, models, onEquipmentSelect },
		ref
	) => {
			...
			<MapContainer
				...
				{positions.map((pos) => {
					...
					const equipmentStateHistory =
						stateHistory?.find((s) => s.equipmentId === pos.equipmentId)
							?.states || [];
					const latestStateId =
						equipmentStateHistory.length > 0
							? equipmentStateHistory[equipmentStateHistory.length - 1]
									?.equipmentStateId
							: undefined;
					const latestState = states?.find((s) => s.id === latestStateId);

					// Calculate productivity percentage
					const productivity = calculateProductivity(equipmentStateHistory);

					// Find the model for the equipment
					const model = models.find((m) => m.id === equipment.equipmentModelId);

					// Calculate earnings based on the model
					const earnings = model
						? calculateEarnings(equipmentStateHistory, model)
						: "N/A";
						...
						<React.Fragment key={pos.equipmentId}>
							...
							<Marker
								...
								<Popup>
									...
									<br />
									<button onClick={() => onEquipmentSelect(equipment)}>
								...
								<Tooltip
									...
									permanent>
									<span>
										{equipment.name}: {latestState?.name || "Desconhecido"}
										<br />
										Produtividade: {productivity.toFixed(2)}%
										<br />
										Ganho: {formatToBRL(earnings)}
									</span>
								</Tooltip>
							</Marker>
						</React.Fragment>
