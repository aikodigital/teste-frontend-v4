...
export const Input: React.FC = () => {
  const { organizedData, setResultFilter, setFilteredData, setFiltered } =
    useEquipmentContext();
  ...
  const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (Array.isArray(organizedData)) {
      const filtered = organizedData.filter((equipment: Equipment) => {
        const nameMatch = equipment.name
          .toLowerCase()
          .includes(value.toLowerCase());
        const modelNameMatch = equipment.modelName
          .toLowerCase()
          .includes(value.toLowerCase());
        const stateNameMatch =
          equipment.states.length > 0
            ? equipment.states[equipment.states.length - 1].stateName
                .toLowerCase()
                .includes(value.toLowerCase())
            : false;
