...
const MapComponent = () => {
    ...
    // Filtrar marcadores por estado ou termo de busca
    ...
    // Carregar as posições mais recentes dos equipamentos
    useEffect(() => {
    ...
    const handleMarkerClick = (equipmentId) => {
        const stateHistory = getEquipmentStateHistory().find(hist => hist.equipmentId === equipmentId);
                    ...
                    >
                        <Popup>
                            <div>
                                ...
                                <button onClick={() => handleMarkerClick(marker.id)}>Ver Histórico</button>
                            </div>
                        </Popup>
                        {/* Historico de posicoes exibido so para o equipamento selecionado */}
                        {selectedEquipment && selectedEquipment.id === marker.id && (
                            <Polyline positions={marker.positions.map(pos => [pos.lat, pos.lon])} color="blue" />
                        )}
                    </Marker>
                ))}
            </MapContainer>
