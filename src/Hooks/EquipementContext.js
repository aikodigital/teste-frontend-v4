...
export const EquipamentProvider = ({ children }) => {
  ...
  const [ dataEquipamentSelected, setDataEquipamentSelected] = useState()
  ...
  const [ equipaments, setEquipaments] = useState(equipamentsData)
  ...
  function findState (id){
    const state = equipmentState.find(i=> i.id === id)
    return state
  }
  ...
  function calculateTimeDiffInHours(start, end) {
    const diffInMs = new Date(end).getTime() - new Date(start).getTime();
    return diffInMs / (1000 * 60 * 60); // Convertendo para horas
  }
  
  // Função para calcular o tempo total por estado
  function calculateTotalTimeByState(equipmentData) {
    // Verifica se o argumento equipmentData é válido
    if (!equipmentData) return null;
    
    // Objeto para armazenar a duração total por estado
    const stateDurations = {};

    // Itera sobre todos os estados, exceto o último
    for (let i = 0; i < equipmentData.states.length - 1; i++) {
        // Estado atual e próximo estado na lista
        const currentState = equipmentData.states[i];
        const nextState = equipmentData.states[i + 1];

        // Calcula a diferença de tempo em horas entre os estados
        const timeDiff = calculateTimeDiffInHours(currentState.date, nextState.date);

        // Encontra as informações do estado usando o equipmentStateId
        const stateInfo = equipmentState.find(state => state.id === currentState.equipmentStateId);

        // Se as informações do estado foram encontradas
        if (stateInfo) {
            // Se o nome do estado ainda não está no objeto stateDurations, inicializa com 0
            if (!stateDurations[stateInfo.name]) {
                stateDurations[stateInfo.name] = {
                  value : 0,
                  id : currentState.equipmentStateId,
                };
            }

            // Adiciona o tempo calculado à duração total para o estado correspondente
            stateDurations[stateInfo.name].value += timeDiff;
        }
    }
