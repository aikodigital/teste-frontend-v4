...
export function getLatestPosition(positionHistory: EquipmentPositionHistory): Position | null {
...
export function getLatestState(
  ...
  return states.find((state) => state.id === latestState.equipmentStateId) || null;
}
...
export function getEquipmentDetails(
  ...
  const positionHistory = positionHistories.find((ph) => ph.equipmentId === equipment.id);
  const stateHistory = stateHistories.find((sh) => sh.equipmentId === equipment.id);
